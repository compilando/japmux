...
import { Tactic } from '@/services/api';
import CopyButton from '../common/CopyButton'; // Asumiendo que se usa

interface TacticsTableProps {
    tactics: Tactic[];
    onEdit: (item: Tactic) => void;
    onDelete: (id: string) => void; // onDelete espera el ID de la Tactic
    loading?: boolean;
}

const TacticsTable: React.FC<TacticsTableProps> = ({ tactics, onEdit, onDelete, loading }) => {
    if (loading) {
        return <p>Loading tactics...</p>;
    }
        ...
        <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        <th scope="col" className="w-3/12 px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">ID</th>
                        ...
                        <th scope="col" className="w-4/12 px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Description</th>
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    {Array.isArray(tactics) && tactics.map((item) => (
                        <tr key={item.id}>
                            ...
                            <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                                <span className="block truncate max-w-md" title={item.description}>{item.description || '-'}</span>
                            </td>
            ...
            {(!Array.isArray(tactics) || tactics.length === 0) && (
                <p className="text-center py-4 text-gray-500 dark:text-gray-400">No tactics found.</p>
            )}
        </div>
    );
};
