...
import { CreateEnvironmentDto, UpdateEnvironmentDto } from '@/services/api'; // Usar los tipos correctos

interface EnvironmentFormProps {
    initialData: CreateEnvironmentDto | null;
    onSave: (data: CreateEnvironmentDto | UpdateEnvironmentDto) => void;
    onCancel: () => void;
}
...
const EnvironmentForm: React.FC<EnvironmentFormProps> = ({ initialData, onSave, onCancel }) => {
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        const payload: CreateEnvironmentDto | UpdateEnvironmentDto = {
            name: name.trim(),
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
                ...
                >
                    {initialData ? 'Save Changes' : 'Create Environment'}
                </button>
            </div>
        </form>
    );
