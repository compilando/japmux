...
const AiModelsPage: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const data = await aiModelService.findAll();
            ...
            setError(`Failed to fetch AI models: ${err instanceof Error ? err.message : String(err)}`);
            setAiModelsList([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleSave = async (payload: CreateAiModelDto | UpdateAiModelDto) => {
        try {
            if (editingModel) {
                await aiModelService.update(editingModel.id, payload as UpdateAiModelDto);
            } else {
                await aiModelService.create(payload as CreateAiModelDto);
            }
    ...
    const breadcrumbs = [
        { label: "Home", href: "/" },
        // { label: "Management", href: "/ai-models" }, // Opcional
        { label: "AI Models" } // Último elemento sin href
    ];
