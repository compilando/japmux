...
import {
    ...
    updatePromptAsset,
    deletePromptAsset,
    PromptAssetCreatePayload,
    PromptAssetUpdatePayload
} from '@/services/api';
...
const PromptAssetsPage: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const data = await getPromptAssets();
    ...
    const handleSave = async (payload: PromptAssetCreatePayload | PromptAssetUpdatePayload) => {
        try {
            if (editingItem) {
                await updatePromptAsset(editingItem.id, payload as PromptAssetUpdatePayload);
            } else {
                await createPromptAsset(payload as PromptAssetCreatePayload);
            }
        ...
        <>
            ...
            {!loading && !error && (
                <div className="bg-white dark:bg-gray-800 shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    <PromptAssetsTable
                        promptAssets={itemsList} // Prop name changed
                        onEdit={handleEdit}
